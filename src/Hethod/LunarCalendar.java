...
public class LunarCalendar {
	...
	public LunarCalendar(Calendar cal) {
		...
		dayCyl = offset + 40;
		monCyl = 14;
		 /* 用offset减去每农历年的天数计算当天是农历第几天,i最终结果是农历的年份,offset是当年的第几天*/
		int iYear, daysOfYear = 0;
		for (iYear = 1900; iYear < 2050 && offset > 0; iYear++) {
			daysOfYear = yearDays(iYear);
			offset -= daysOfYear;           // 用offset减去每农历年的天数
			monCyl += 12;
		}
		if (offset < 0) {
			offset += daysOfYear;
    ...
    public String getnian(){//农历年
    	return cyclical();
    	
    }
    
    public String getShengxiao(){//农历生肖
       return animalsYear();	
    }

    public String getyue(){//农历月
    	if(month - 1 == 0){
    		chineseNumber[0] = "正";
    	}
